          ...
          <Section title={translations.nav.about[lang]} icon={<UserIcon />}>
            <div className="bg-teal-800 p-8 rounded-lg border border-teal-700 shadow-inner" style={{
              ...
              <div className="flex flex-col md:flex-row items-center gap-8 text-center md:text-start rtl:md:text-right">
                <img src="https://picsum.photos/seed/green-nature/200/200" alt={data.studentInfo.name} className="w-48 h-48 rounded-full border-4 border-amber-500 object-cover shadow-lg" />
                <div className="flex-1">
                  ...
                  />
                  <div className="text-xl text-amber-300 mt-1">
                    <Editable
                   ...
                   <Editable
                      ...
                      tag="p"
                      as="textarea"
                      className="mt-4 text-lg text-amber-200 leading-relaxed"
                    />
                </div>
              </div>
            </div>
        ...
        return (
          <Section title={translations.nav.project[lang]} icon={<ProjectIcon />}>
            <div className="bg-teal-800 rounded-lg overflow-hidden shadow-lg border border-teal-700">
              ...
              <div className="p-8">
                <Editable value={project.title[lang]} onSave={v => handleDataChange(`featuredProject.title.${lang}`, v)} tag="h2" className="text-4xl font-black text-amber-500 mb-2" />
                <Editable value={project.description[lang]} onSave={v => handleDataChange(`featuredProject.description.${lang}`, v)} tag="p" as="textarea" className="text-lg text-amber-300 mb-6" />
            ...
            <div className="space-y-8">
              {data.evaluations.map((evalItem, index) => (
                <blockquote key={evalItem.id} className="bg-teal-800 p-6 rounded-lg border-l-4 border-amber-500 rtl:border-l-0 rtl:border-r-4">
                  <Editable value={evalItem.comment[lang]} onSave={v => handleDataChange(`evaluations.${index}.comment.${lang}`, v)} as="textarea" tag="p" className="text-lg italic text-amber-200" />
                  <footer className="mt-4">
                    ...
                    <Editable value={evalItem.role[lang]} onSave={v => handleDataChange(`evaluations.${index}.role.${lang}`, v)} tag="p" className="text-sm text-amber-300" />
                  </footer>
                </blockquote>
              ))}
            </div>
