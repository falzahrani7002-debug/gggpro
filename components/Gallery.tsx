...
const Gallery: React.FC = () => {
  ...
  const FilterButton: React.FC<{onClick: () => void, isActive: boolean, children: React.ReactNode}> = ({onClick, isActive, children}) => (
      <button 
        onClick={onClick}
        className={`px-4 py-2 text-sm font-medium rounded-md transition-colors duration-200 ${isActive ? 'bg-yellow-400 text-black' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}>
        {children}
      </button>
  );
  ...
  return (
    <div>
      <div className="flex flex-wrap gap-4 mb-8">
        <div>
          <label className="block text-sm font-medium text-gray-400 mb-2">{translations.filterByType[lang]}</label>
         ...
         <div>
          <label className="block text-sm font-medium text-gray-400 mb-2">{translations.filterByYear[lang]}</label>
          <div className="flex flex-wrap gap-2">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredItems.map(item => (
          <div key={item.id} className="group relative overflow-hidden rounded-lg shadow-lg bg-gray-800 border border-gray-700">
            <a href={item.url} target="_blank" rel="noopener noreferrer">
              ...
              <div className="p-4">
                <h3 className="font-bold text-lg text-yellow-400 truncate">{item.title[lang]}</h3>
                <p className="text-sm text-gray-400">{item.description[lang]}</p>
                <span className="absolute top-2 right-2 rtl:right-auto rtl:left-2 bg-blue-600 text-white text-xs font-semibold px-2 py-1 rounded-full">{item.year}</span>
              </div>
            </a>
          </div>
        ))}
